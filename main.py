...
def create_dataset(directory, label):
    ...
    print(f"Filenames in {directory}: {[os.path.basename(path) for path in audio_files]}")
    return X, y
...
def train_model(X, y):
    ...
    if X_test is not None:
        ...
        print("Insufficient samples for stratified splitting. Train on all available data.")
        svm_classifier = SVC(kernel='linear', random_state=42)
        svm_classifier.fit(X_train_scaled, y_train)
...
def main():
    genuine_dir = r"C:\Users\ADMIN\Downloads\DeepFake-Audio-Detection_miniproject\DeepFake-Audio-Detection_miniproject\real_audio"
    deepfake_dir = r"C:\Users\ADMIN\Downloads\DeepFake-Audio-Detection_miniproject\DeepFake-Audio-Detection_miniproject\deepfake_audio"
...
if __name__ == "__main__":
    main()
    user_input_file = input(r"C:\Users\ADMIN\Downloads\DeepFake-Audio-Detection-MFCC-main\toanalyze")
